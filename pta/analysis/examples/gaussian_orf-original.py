...
# --- stringlike Gaussian source injection ---
# The injection is elliptical in the tangent plane.  The analytic ORF below is
# axisymmetric, so we store an effective scalar sigma and kappa truth for the fit.
nsrc = 100
sigma_x_deg = 1.0
sigma_y_deg = 0.3
pa_deg = 0.0
...
ax.plot(lon_src, lat_src, "*", ms=14, label="GW source center")
...
gwphi = parameter.UniformCyclic(0, 2 * np.pi)("gw_phi")
kappa = parameter.Uniform(0.1, 20000.0)("kappa")
...
corner_names = ["gw_phi", "gw_theta", "kappa"]
corner_idxs = [pta.param_names.index(name) for name in corner_names]

fig1 = corner.corner(
    chain[burn:, :ndim][:, corner_idxs],
    40,
    labels=corner_names,
    smooth=True,
    truths=[gwphi0, gwtheta, kappa_true]
)
...
print(f"Gaussian injection: sigma_x={sigma_x_deg} deg, sigma_y={sigma_y_deg} deg, sigma_eff={sigma_eff_deg} deg, kappa_true={kappa_true}", flush=True)
